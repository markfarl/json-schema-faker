...
function () {
  ...
  _createClass(Registry, [{
    ...
    key: "registerMany",
    value: function registerMany(formats) {
      var _this = this;

      Object.keys(formats).forEach(function (name) {
        _this.data[name] = formats[name];
      });
    }
    /**
...
/*#__PURE__*/
function (_Registry) {
  _inherits(OptionRegistry, _Registry);

  function OptionRegistry() {
    var _this;

    _classCallCheck(this, OptionRegistry);

    _this = _possibleConstructorReturn(this, _getPrototypeOf(OptionRegistry).call(this));
    _this.data = Object.assign({}, defaults);
    _this._defaults = defaults;
    return _this;
  }

  _createClass(OptionRegistry, [{
    key: "defaults",
    get: function get() {
...
function number(min, max, defMin, defMax) {
  var hasPrecision = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
...
function _short(schema) {
  var s = JSON.stringify(schema);
  var l = JSON.stringify(schema, null, 2);
  return s.length > 400 ? "".concat(l.substr(0, 400), "...") : l;
}
  ...
  Object.keys(obj).forEach(function (k) {
    if (props.indexOf(k) === -1) {
      if (Array.isArray(obj[k])) {
        ...
        copy[k] = _typeof(obj[k]) === 'object' ? merge({}, obj[k]) : obj[k];
      }
    }
  });
  return copy;
...
function proxy(gen) {
  return function (value, schema, property, rootSchema) {
    ...
    value = _typeof(ctx) === 'object' ? ctx[props[0]] : ctx; // invoke dynamic generators

    if (typeof value === 'function') {
      value = value.apply(ctx, args.map(function (x) {
    ...
    if (Object.prototype.toString.call(value) === '[object Object]') {
      Object.keys(value).forEach(function (key) {
        if (typeof value[key] === 'function') {
          throw new Error("Cannot resolve value for '".concat(property, ": ").concat(fn, "', given: ").concat(value));
        }
      });
    }
  ...
  _createClass(Container, [{
    ...
    value: function get(name) {
      if (typeof this.registry[name] === 'undefined') {
        throw new ReferenceError("'".concat(name, "' dependency doesn't exist."));
      }
      ...
      var _loop = function _loop() {
        ...
        if (typeof gen === 'function') {
          Object.defineProperty(schema, 'generate', {
            ...
            enumerable: false,
            writable: false,
            value: function value(rootSchema) {
...
function arrayType(value, path, resolve, traverseCallback) {
  ...
  if (!(value.items || value.additionalItems)) {
    if (utils.hasProperties(value, 'minItems', 'maxItems', 'uniqueItems')) {
      throw new ParseError("missing items for ".concat(utils["short"](value)), path);
    }
  ...
  var sample = _typeof(value.additionalItems) === 'object' ? value.additionalItems : {};
...
function objectType(value, path, resolve, traverseCallback) {
  ...
  while (fillProps) {
    ...
    if (allowsAdditional) {
      if (reuseProps && propertyKeys.length - current > min) {
        ...
        var _word = get() || wordsGenerator(1) + random.randexp('[a-f\\d]{1,3}');

        if (!props[_word]) {
          props[_word] = additionalProperties || anyType;
          current += 1;
        }
      }
    }
    ...
    for (var i = 0; current < min && i < patternPropertyKeys.length; i += 1) {
      ...
      var _word2 = random.randexp(_key);

      if (!props[_word2]) {
        props[_word2] = patternProperties[_key];
        current += 1;
      }
    }
  }
  ...
  if (!allowsAdditional && current < min) {
    if (missing.length) {
      throw new ParseError("properties '".concat(missing.join(', '), "' were not found while additionalProperties is false:\n").concat(utils["short"](value)), path);
    }
...
var URI_PATTERN = "https?://{hostname}(?:".concat(FRAGMENT, ")+");
...
var regexps = {
  ...
  'uri-reference': "".concat(URI_PATTERN).concat(PARAM_PATTERN),
  'uri-template': URI_PATTERN.replace('(?:', '(?:/\\{[a-z][:a-zA-Z0-9-]*\\}|'),
  'json-pointer': "(/(?:".concat(FRAGMENT.replace(']*', '/]*'), "|~[01]))+"),
...
var ALLOWED_FORMATS = new RegExp("\\{(".concat(Object.keys(regexps).join('|'), ")\\}"));
/**
    ...
    default:
      if (typeof callback === 'undefined') {
        if (optionAPI('failOnInvalidFormat')) {
          throw new Error("unknown registry key ".concat(utils["short"](value.format)));
        } else {
          return invalid();
        }
      }

      throw new Error("unsupported format '".concat(value.format, "'"));
  }
}
...
function traverse(schema, path, resolve, rootSchema) {
  ...
  if (schema.not && _typeof(schema.not) === 'object') {
  ...
  if (typeof type === 'string') {
    if (!typeMap[type]) {
      if (optionAPI('failOnInvalidTypes')) {
        throw new ParseError("unknown primitive ".concat(utils["short"](type)), path.concat(['type']));
      } else {
...
function resolve(obj, data, values, property) {
  ...
  if (obj.jsonPath) {
    var params = _typeof(obj.jsonPath) !== 'object' ? {
      path: obj.jsonPath
    } : obj.jsonPath;
    ...
    var key = "".concat(params.group, "__").concat(params.path);
  ...
  try {
    var result = traverse(utils.merge({}, schema), [], function reduce(sub, maxReduceDepth, parentSchemaPath) {
      ...
      if (typeof sub.$ref === 'string') {
        ...
        if (typeof ref !== 'undefined') {
          if (!ref && optionAPI('ignoreMissingRefs') !== true) {
            throw new Error("Reference not found: ".concat(sub.$ref));
          }
      ...
      Object.keys(sub).forEach(function (prop) {
        if ((Array.isArray(sub[prop]) || _typeof(sub[prop]) === 'object') && !utils.isKey(prop)) {
    ...
    if (e.path) {
      throw new Error("".concat(e.message, " in /").concat(e.path.join('/')));
    } else {
      throw e;
    }
  }
  ...
  container.define('sequentialDate', function sequentialDate(value, schema) {
    ...
    if (value) {
      ...
      if (['seconds', 'minutes', 'hours', 'days', 'weeks', 'months', 'years'].indexOf(value) === -1) {
        throw new Error("Unsupported increment by ".concat(utils["short"](value)));
      }
...
var jsf = function jsf(schema, refs, cwd) {
